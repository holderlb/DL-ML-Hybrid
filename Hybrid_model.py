...
DATADIR = '/home/mavaie/backup/datasets/'
...
def create_one_hot_encoder_sequences(x_ndmr):
    ...
    ndmr_data = np.zeros ((len (x_ndmr), 1000, 5))
    for i in range (0, len (x_ndmr)):
...
def create_data_and_labels(chrm_index, threshold, threshold_ndmr, least_set):
    ...
    x_ndmr = extract_dna_sequences (first_rec, y, 1000)
    x_dmr = extract_dna_sequences (first_rec, x, 1000)
...
def train_hybrid_model(data_seq, labels, layer_num = 5):
	"""
	Train a hybrid model ecaluate the performance and return the hybrid model 
	Parameters
	----------
	@param labels: labels 
...
def hybrid_nfold_cross_validation(data_seq, labels, nfold=5, layer_num= 5):
    ...
    for ifold in range (0, (nfold)):
        ...
        xgb_classifier = fit_ml_classifier (out_array, train_ground, classifier_case='XGB')
        out_array = feature_extractor_from_dl_model (model, test_X, layer_num)
        # Evaluate the perforamce of the model
...
def main():
    ...
    if args.option not in ['train-hybrid', 'test-hybrid' '5-fold-hybrid', 'visualize']:
        print('invalid option: ', args.option)
        print("Please input a option: train-hybrid, test-hybrid, 5-fold-hybrid, or visualize")
    ...
    elif args.option== '5-fold-hybric':
