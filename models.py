...
chrm_index = 'X'
# A threshold for choosing a p-value for sampling DMRs: p-value < threshold
threshold = 0.000000000000001
# A threshold for choosing a p-value for sampling non-DMRs: p-value > (1 - threshold_ndmr)
threshold_ndmr = 0.00000001

# Input_shape is used for the "input_shape" parameter of the first layer of DL network.
# It is a 2D array where the first element shows the length of the sequence and second
# element is the number of characters used for the representation (one-hot encoding).
INPUT_SHAPE = [1000,5]
...
def extract_dna_sequences(record, label_indices, seq_len):
    ...
    @seq_len: The length of each sequecne
    """
    x_data = []
...
def create_data_and_labels(chrm_index, threshold, threshold_ndmr, least_set):
    ...
    @param chrm_index: chromosome number 
    @param threshold: threshold for choosing DMRs 
    @param threshold_ndmr: threshold for choosing non-DMRs
    @param least_set: The number of exposure that a sample is non-DMR in at least this value exposure
    """
...
def visualization_for_a_node(data, labels, model, layer_name="conv1d_2"):
    ...
    data = Data(['./output_negative.fasta', './output_positive.fasta'], "ACGTN")
...
def train_hybrid_model(data_seq, labels, layer_num = 5):
    """
    ...
    evaluate_models(xgb_classifier, out_array, test_ground)
...
def hybrid_nfold_cross_validation(data_seq, labels, nfold=5, layer_num=5):
    show_distribution_labels (labels)
...
def main():
    ...
    parser.add_argument('--option', dest='option', type=str, default='train',
                        help='options: train-hybrid, train-DL, test-DL, test-hybrid, 5-fold-hybrid, 5-fold-DL, or visualize')
    args = parser.parse_args()
    if args.option not in ['train-hybrid','test-hybrid','5-fold-hybrid','trainDL','test-DL','5-fold DL','visualize']:
        print('invalid option: ', args.option)
        print("Please input a option: train-hybrid, test-hybrid, 5-fold-hybrid, train-DL, test-DL, 5-fold-DL, or visualize")
    elif args.option == 'train-hybrid':
       train_hybrid_model(data_seq, labels, layer_num = 5)
    elif args.option == 'test-hybrid':
        classifier = load_model('model_log')
        evaluate_models(classifier, data_seq, labels)
    elif args.option == '5-fold-hybrid':
        hybrid_nfold_cross_validation(data_seq, labels)
