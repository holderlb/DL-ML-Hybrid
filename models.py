...
# TODO: change CV to n-fold CV
def dl_nfold_cross_validation(data_seq, labels, nfold=5):
    global INPUT_SHAPE
    # Create the model and fit it with the data
    model = build_dl_model (INPUT_SHAPE, nb_classes=2)
    print (model.summary ())
    accuracy_list = []
    f1_score_list = []
    precision_list = []
...
def main():
    ...
    parser.add_argument('--option', dest='option', type=str, default='train-hybrid',
                        help='options: train-hybrid, train-DL, test-DL, test-hybrid, 5-fold-hybrid, 5-fold-DL, or visualize')
    args = parser.parse_args()
    if args.option not in ['train-hybrid','test-hybrid','5-fold-hybrid','train-DL','test-DL','5-fold DL','visualize']:
        print('invalid option: ', args.option)
        print("available optons: train-hybrid, test-hybrid, 5-fold-hybrid, train-DL, test-DL, 5-fold-DL, or visualize")
        sys.exit()
    # Create data set
    ...
    # Check that #dmrs and #non-dmrs are both non-zero
    ...
    if args.option == 'train-hybrid':
       train_hybrid_model(data_seq, labels)
    elif args.option == 'test-hybrid':
        model = load_model(OUTPUT_DIR + MODEL_LOG_FILE)
        evaluate_model(model, data_seq, labels)
